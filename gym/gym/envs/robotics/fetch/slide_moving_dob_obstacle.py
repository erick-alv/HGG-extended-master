...
MODEL_XML_PATH = os.path.join('fetch', 'slide_moving_dob_obstacle.xml')
...
class FetchSlideMovingObstacleEnv(fetch_env.FetchEnv, utils.EzPickle):
    def __init__(self, reward_type='sparse'):
        ...
        self.target_goal_center = np.array([1.7, 0.75, 0.42])
        ...
        fetch_env.FetchEnv.__init__(
            ...
            obj_range=np.array([0.02, 0.08]),#np.array([0.02, 0.08]),
            target_range=np.array([0.1, 0.15]), distance_threshold=0.05,
    ...
    def test_setup(self, new_vel_lims=[0.9, 1.]):
        '''
