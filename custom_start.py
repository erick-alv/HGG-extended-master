...
def get_args_and_initialize():
    ...
    parser.add_argument('--batch_size', help='batch_size for training', type=int, default=2048)
    parser.add_argument('--training_steps', help='how many times agents agent train in a step', type=int, default=1)
    ...
    if hasattr(args, 'type_experiment'):
        if args.type_experiment == 'tdm_training':
        ...
        elif args.type_experiment == 'vae_agent':
            ...
            args.csv_logger = CSV_Logger(['critic1_loss', 'critic2_loss', 'actor_loss','episode_reward', 'distance_to_goal','success', 'episode_steps',
                                          'latent_distant_sub', 'distance_sub','time','estimator_loss', 'estimator_rl_loss'],
                                         args, iteration_fieldnames=['epoch','episode'])
            ...
            args.goal_type = 'goal_space'#'state', 'latent','concat'
            ...
            args.training_episodes = 30
            args.env_steps = 100
            args.observation_type = 'real'#,'concat'
            args.goal_type = 'goal_space'#'state', 'latent','concat'
            args.training_steps = 1
