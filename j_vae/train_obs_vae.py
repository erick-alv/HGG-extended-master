...
img_size = 84
n_path = '../data/FetchPush/vae_model_obs'
...
class VAE(nn.Module):
    def __init__(self):
        ...
        # Try to reduce
        self.fc21 = nn.Linear(400, 22)
        self.fc22 = nn.Linear(400, 22)
        self.fc3 = nn.Linear(22, 400)
...
def train(epoch, model, optimizer, device, log_interval, batch_size):
    model.train()
    train_loss = 0
    data_set = np.load('../data/FetchPush/obs_set.npy')
...
def test(epoch, model, test_loader, batch_size, device):
    model.eval()
    test_loss = 0
    with torch.no_grad():
