...
class VAE_SB(nn.Module):
    def __init__(self, device, img_size=84, latent_size=2,full_connected_size=640, input_channels=3,
...
def train_max_separation(epoch, model, optimizer, device, log_interval, batch_size):
    ...
    while current_processed < total_pairs:
        ...
        idx1 = np.random.choice(all_idx, size=batch_size, replace=False)
        is1 = data_set[idx1]
        for d in idx1:
            all_idx.remove(d)
...
if __name__ == '__main__':
    ...
    train_Vae(batch_size=32, epochs=50, load=False, latent_size=15)
    # test_VAE_SB(device)
    # show_1d_manifold()
    #show_2d_manifold(84)
