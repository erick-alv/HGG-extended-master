...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, goal_dim, rem_steps_dim, max_action, networks_hidden):
        ...
        self.l_in = nn.Linear(state_dim + goal_dim + rem_steps_dim, networks_hidden[0])
        last_dim = networks_hidden[0]
        self.hidden_layers = []
        for i in range(networks_hidden):
            self.hidden_layers.append(nn.Linear(last_dim, ))

        [nn.Linear(300, 300)]
        self.l_out = nn.Linear(networks_hidden[-1], action_dim)

        self.max_action = max_action

    def forward(self, state):
        a = F.relu(self.l_in(state))
